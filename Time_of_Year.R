###############################################################################
# Snowy Owl Time-of-Year Analyses
...
# Helper function: convert fiscal day to a "true" date for reference year
fiscal_day_to_date <- function(day_fiscal, ref_year = 2024) {
...
# =============================================================================
# 📁 5. All Years Box and Whisker Plot 
# -----------------------------------------------------------------------------
# Horizontal boxplot of all snowy owl events aligned to Sept–Aug fiscal year.
# =============================================================================
...
summary_all <- dat %>%
  ...
  mutate(
    date_min    = fiscal_day_to_date(min),
    date_q1     = fiscal_day_to_date(q1),
    date_median = fiscal_day_to_date(median),
    date_q3     = fiscal_day_to_date(q3),
    date_max    = fiscal_day_to_date(max)
  )

# Save summary table with true dates
summary_all %>%
  select(date_min, date_q1, date_median, date_q3, date_max) %>%
  write_csv("SNOW_BoxSummary_AllEvents_TrueDates.csv")

# Plot horizontal boxplot
ggplot(dat, aes(x = fiscal_day_of_year, y = 1)) +
  geom_boxplot(
    fill = "skyblue", alpha = 0.5, outlier.shape = NA, width = 0.4,
    color = "black", median.color = "purple"
  ) +
  geom_jitter(height = 0.1, alpha = 0.5, color = "darkblue", size = 1.3) +
  scale_x_continuous(
    breaks = month_breaks,
...
# Save high-definition image scaled for Word document
ggsave("snowy_owl_activity_distribution.tiff", width = 6.5, height = 5, dpi = 600)



# =============================================================================
# 📁 6. Boxplot by Migration Period
# -----------------------------------------------------------------------------
...
# =============================================================================
# 📁 7. Summary Table of Migration Timing
# -----------------------------------------------------------------------------
...
# =============================================================================
# 📁 8. Ridge Plots by Migration Period
# -----------------------------------------------------------------------------
